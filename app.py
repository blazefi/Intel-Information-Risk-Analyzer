        ...
        if filenames:
            for i in filenames:
                with open(dirpath + '/' + i) as f:
                    ...
                    keyword = [word for word in keyword if word not in lst_stopwords]
                    lem = nltk.stem.wordnet.WordNetLemmatizer()
...
def update_scores(df, info, country, month):
    ...
    if 'importance' in info:
        ...
        if info[idx+1] == 'high':
            val = df.loc[(df['country'] == country) & (df['month'] == month), 'importance']
            df.loc[(df['country'] == country) & (df['month'] == month), 'importance'] = val + 0.5

    if 'level' in info:
        idx = info.index('level')
        if info[idx+1] == 'notice':
            val = df.loc[(df['country'] == country) & (df['month'] == month), 'level']
            df.loc[(df['country'] == country) & (df['month'] == month), 'level'] = val + 0.5

        elif info[idx+1] == 'advisory':
            df.loc[(df['country'] == country) & (df['month'] == month), 'level']
            df.loc[(df['country'] == country) & (df['month'] == month), 'level'] = val + 1
    ...
    if 'severity' in info:
        ...
        val = df.loc[(df['country'] == country) & (df['month'] == month), 'severity']

        if info[idx+1] == 'score':
            try:
                df.loc[(df['country'] == country) & (df['month'] == month), 'severity'] = val + w2n.word_to_num(info[idx+2])
            ...
            try:
                if info[idx+1] == 'minor':
                    df.loc[(df['country'] == country) & (df['month'] == month), 'severity'] = val + 0.5

                elif info[idx + 1] == 'moderate':
                    df.loc[(df['country'] == country) & (df['month'] == month), 'severity'] = val + 1

                elif info[idx + 1] == 'severe':
                    df.loc[(df['country'] == country) & (df['month'] == month), 'severity'] = val + 2
...
def update_risks(df, risks, country, month):
    ...
    for risk in risks:
        ...
        df.loc[(df['country'] == country) & (df['month'] == month), risk] = val + 1
...
def main():
    ...
    if choice == "Home":
        ...
        risk_avg = (country_df.groupby('country')['severity'].mean() + country_df.groupby('country')['level'].mean())/2. 
        values = []
        iso_alpha = []
        for col in df['country'].values:
            for i in risk_avg.index:
                if i == col.lower():
                    ...
                    iso_alpha.append(df[df['country'] == col]['iso_alpha'].values[0])
            ...
            if docx_files is not None:
                for docx_file in docx_files:
                    ...
                    if docx_file.type == "text/plain":
                        ...
                        #preprocessed_text = preprocess_pdf_text(raw_text)
                        #categories = select_category(preprocessed_text)
                        # st.write(categories)
                        ...
                        try:
                            ...
                            st.write(text)
                            preprocessed_text = preprocess_pdf_text(text)
                            #categories = select_category(preprocessed_text)
                            categories = select_category(preprocessed_text)
                            st.write(categories)
                            ...
                            df = update_scores(
                                df, preprocessed_text, country, month)
    ...
    if choice == "Upload Image":
        ...
        if image_file is not None:
            ...
            try:
                preprocessed_text = preprocess_pdf_text(ocr_text)
                categories = select_category(preprocessed_text)
                st.write(categories)
                country, month = extract_country_and_month(ocr_text)
                st.write("Country: " + country)
                st.write("Month: " + month)
                alert_type = type_of_alert(ocr_text)
                st.write("Type of alert: " + alert_type)

                df = update_risks(df, categories, country, month)
                df = update_scores(df, preprocessed_text, country, month)
